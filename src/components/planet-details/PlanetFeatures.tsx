...
// Version simplifiée des variants d'animation
const featureVariants = {
  ...
  visible: (i: number) => ({ 
    ...
    y: 0,
    transition: { 
      delay: i * 0.05, // Réduit le délai entre les éléments
      duration: 0.3,
      ease: "easeOut"
    } 
  }),
...
const FeatureCard: React.FC<FeatureCardProps> = ({ feature, index }) => {
  ...
  // Si l'utilisateur préfère les animations réduites, désactiver les animations
  if (prefersReducedMotion) {
    return (
      <div className="relative bg-gray-50/50 dark:bg-gray-900/50 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-800 shadow-lg backdrop-blur-sm h-fit">
        ...
        <div className="p-5 relative z-10">
          <div className="flex items-center justify-between mb-3">
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" style={{
...
export function PlanetFeatures({ planetName }: { planetName: string }) {
  ...
  const containerVariants = {
    ...
    visible: { 
      opacity: 1,
      transition: {
        staggerChildren: 0.08, // Réduit le délai entre les éléments enfants
        delayChildren: 0.05
      }
    }
  };
  
  // Variants d'animation simplifiés pour le titre
  const titleVariants = {
    hidden: { opacity: 0, y: -10 },
    visible: { 
      opacity: 1, 
  ...
  // Version simplifiée sans animations pour ceux qui préfèrent des animations réduites
  if (prefersReducedMotion) {
    return (
      <div className="bg-gray-50/50 dark:bg-gray-900/50 backdrop-blur-md rounded-xl p-6 border border-gray-200 dark:border-gray-800 shadow-lg relative overflow-hidden">
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {/* Limiter le nombre de caractéristiques affichées pour améliorer les performances */}
          {features.slice(0, 6).map((feature, index) => (
            <FeatureCard key={feature.id} feature={feature} index={index} />
          ))}
        </div>
